...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Extraction de l'ID de page depuis le webhook Notion
    let pageId = req.body.pageId; // Format direct
    
    // Si c'est un webhook Notion, l'ID peut être dans différents endroits
    if (!pageId && req.body.page_id) {
      pageId = req.body.page_id;
    }
    
    // Format webhook Notion avec object
    if (!pageId && req.body.object && req.body.object.id) {
      pageId = req.body.object.id;
    }
    
    // Format webhook Notion avec page
    if (!pageId && req.body.page && req.body.page.id) {
      pageId = req.body.page.id;
    }
    
    if (!pageId) {
      return res.status(400).json({ 
        error: 'Page ID not found in request',
        received: req.body 
      });
    }
